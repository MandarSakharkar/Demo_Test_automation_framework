...
public class RestStepDefs extends BaseStepDefs implements En {
    @Autowired
    ...
    public RestStepDefs() {
        Given("user accesses the {string} api", (String apiName) -> {
        ...
        When("user performs a get request for {string} endpoint", (String endpoint) -> {
            restConnectionUtils.getRequestPojo(endpoint);
        });
        When("user performs a get request for {string}", (String endpoint) -> {
        ...
        Then("user validates that status code is {int}", (Integer statusCode) -> {
            Assert.assertTrue(restParameters.getStatusCode() == statusCode);

        });

        And("validate data from the response based on {string}", (String criteria) -> {
        ...
        When("user updates json request nodes for {string} endpoint", (String endPoint, DataTable dataTable) -> {
        ...
        And("fetch data from response for {string} for districts", (String fetchCategory) -> {
